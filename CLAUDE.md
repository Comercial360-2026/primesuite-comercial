...
4. **Crear siempre un Pull Request contra `main`** una vez subida la rama.
   ...
   ```
5. **Validar los cambios con el Deploy Preview de Netlify** que se genera
   automáticamente en el PR. No dar una tarea por terminada sin ese preview:
   incluir la URL del Deploy Preview en el resumen entregado al usuario.
6. **No hacer merge ni desplegar a producción bajo ninguna circunstancia**,
   salvo que el usuario escriba exactamente:

   > HAZ DEPLOY A PRODUCCIÓN

   Sin ese texto literal: no `git merge` hacia `main`, no `gh pr merge`,
   no push a `main`, no publicar deploy en Netlify. Un "ya está bien",
   "adelante", "mergea" o similar **no** cuenta como autorización.
