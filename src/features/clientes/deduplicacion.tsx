...
// negocios distintos por error es peor que dejar un duplicado. No se puede
// deshacer con comodidad: no queda registro de qué venía de la ficha
// absorbida, por eso el paso de confirmación con el recuento.
...
export function Deduplicacion() {
  ...
  const { data: clientes } = useQuery({
    ...
    queryFn: async () => {
      const { data, error: err } = await supabase
        .from('cliente')
        .select('id, nombre, creado_por, creado_en, estado_fusion');
      if (err) throw err;
  ...
  }, [clientes, conteoVisitas, conteoOportunidades]);
  ...
  const cargando = !clientes || !conteoVisitas || !conteoOportunidades;
          ...
          <div key={g.clave} className="card">
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 'var(--text-xs)', color: 'var(--ink-400)' }}>
                      ...
                      {c.oportunidades === 1 ? '' : 'es'}
                      {c.creado_por && nombresComerciales?.[c.creado_por]
                        ? ` · creada por ${nombresComerciales[c.creado_por]}`
                        : ''}
                    </div>
                  </div>
                </label>
            ...
            {enConfirmacion ? (
              ...
              <button
                type="button"
                className="btn btn-primary"
                style={{ marginTop: 10, width: 'auto', padding: '0 16px' }}
                disabled={bloqueado}
                onClick={() => {
                  setError(null);
