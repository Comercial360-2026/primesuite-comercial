...
// mano con el botón. El agente tarda 6-8 min, así que aquí solo se lee la
...
export function BriefingHoja({ visitaId, clienteId, clienteNombre, onCerrar }: BriefingHojaProps) {
  ...
  if (b?.estado === 'pendiente') {
    estadoTexto = tope?.alcanzado
      ...
      : `En cola desde hace ${minutos(b.pedido_en)} min. Suele tardar 6-8 minutos en total.`;
  } else if (b?.estado === 'generando') {
    estadoTexto = `Generando: lleva ${minutos(b.iniciado_en)} min. Suele tardar 6-8 minutos.`;
  }
          ...
          {b?.contenido ? (
            ...
            b?.estado !== 'error' && (
              <EstadoLista
                estado="vacio"
                icono="briefing"
                mensaje="Aún no hay briefing para esta visita. El agente tarda unos 6-8 minutos en prepararlo."
              />
            )
          )}
          {boton}
