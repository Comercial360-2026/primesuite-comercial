...
const _PANTALLAS = {
  ...
  'visita-activa': {
    ...
    queEs:
      'La pantalla desde la que capturas todo mientras estás con el cliente. Lo primero y en grande, «Captura lo que veas»: cuatro botones iguales —foto, audio, anotar, próximo paso—. «Anotar» es para todo lo que ves y quieres dejar dicho: escribes o dictas y se guarda como nota; si además es un hallazgo («algo que tienen») o una oportunidad («algo para venderles»), lo marcas ahí mismo. Un hallazgo puede llevar una categoría del catálogo (Hardware, Software…), opcional. Debajo, el contexto: el objetivo con el que ibas (tócalo para matizarlo) y dos botones para interlocutores y equipo. Más abajo, «En esta visita», con todo lo capturado (lo tuyo y lo de tus compañeros) en una sola lista, y al final «Cerrar visita». Si estás recorriendo instalaciones, «Marcar zonas» saca una casilla para atar cada captura al sitio; si no, la captura no se ata a ninguna zona.',
    cuando:
  ...
  'detalle-hallazgo': {
    ...
    titulo: 'Hallazgo',
    queEs:
      'Algo que has observado en el cliente y quieres dejar registrado: su texto, su categoría del catálogo (Hardware, Software…), en qué zona estaba y, si aplica, una fecha relevante.',
    cuando:
      'Para completar o corregir un hallazgo. Se crea durante la visita desde «Anotar»; la categoría del catálogo es opcional y puedes cambiarla aquí después. Con «Esto es» puedes devolverlo a nota o pasarlo a oportunidad —el texto y todo lo demás se conservan; la categoría se guarda y vuelve si lo marcas otra vez como hallazgo—. «Archivar» lo saca de la lista de hallazgos del proyecto cuando ya no es vigente, sin borrarlo —sigue en su visita y en el informe de esa visita— y se puede desarchivar.',
